...
- `autobib info` format with default report `--report all` has changed.
  Note that `autobib info --report all` is not intended to be machine-readable; use `autobib info --json` instead.
...
- Internal database changes:
  ...
  - The binary data format of the `data` column in the Records table has changed from the `v0` format to a new `v1` format.
    Visit the [new entry data repository](https://github.com/autobib/autobib-entry) for details on the new format.
